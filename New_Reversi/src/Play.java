...
public class Play {
	...
	public static void main(String[] args) throws IOException {
		...
		switch (task) {
			...
			case 2:
				max("root",startState, 0);
				printBoard(bestChildBoard);
				PrintWriter output2= new PrintWriter(new BufferedWriter(new FileWriter("output.txt",true)));
				output2.println("Node,Depth,Value");
				output2.println(sb);
				output2.close();
			...
			case 3:
				...
				printBoard(bestChildBoard);
				PrintWriter output3= new PrintWriter(new BufferedWriter(new FileWriter("output.txt",true)));
				output3.println("Node,Depth,Value,Alpha,Beta");
				output3.println(sb);
				output3.close();
	...
	private static void printBoard(int [][] board) throws IOException{
		...
		for(int i=0;i<8;i++){
			for(int j=0;j<8;j++){
				output.write(board[i][j]==0?"*":board[i][j]==1?"X":"O");
